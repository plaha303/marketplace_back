...
@shared_task(name="order_service.tasks.send_order_status_update_email", bind=True, max_retries=3, default_retry_delay=60)
def send_order_status_update_email(self, order_id, status):
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to fetch payment status for order {order_id}: {str(e)}")
        ...
        try:
            ...
            logger.warning(f"Failed to fetch shipping status for order {order_id}: {str(e)}")
