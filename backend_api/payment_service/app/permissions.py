    ...
    def has_object_permission(self, request, view, obj):
        ...
        try:
            ...
            if 'admin' in user_roles:
                ...
                return True
            # Перевірка власності для Order
            if hasattr(obj, 'customer_id'):
                result = obj.customer_id == request.user.id
            ...
            logger.error(
                f"Database error in {view.__class__.__name__} for user {request.user.id}, object {obj.__class__.__name__}: {e}")
            return False
        except Exception as e:
            logger.critical(
                f"Unexpected error in {view.__class__.__name__} for user {request.user.id}, object {obj.__class__.__name__}: {e}")
            return False