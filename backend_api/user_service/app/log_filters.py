...
class SensitiveDataFilter(logging.Filter):
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        # Виправлення: очищаємо record.args, якщо вони не потрібні
        if record.args:
            # Перевіряємо, чи msg містить специфікатори форматування
            if '%s' not in msg and '%d' not in msg and '%f' not in msg:
                record.args = ()  # Очищаємо аргументи, якщо вони не використовуються
            else:
                # Обробляємо аргументи, якщо вони є
